...
.directive 'targetGradePieChart', ->
  ...
  scope:
    ...
    showLegend: '=?'
    height: '=?'
  controller: ($scope, $timeout, gradeService, VisualisationService) ->
    ...
    [$scope.options, $scope.config] = VisualisationService.createVisualisation 'pieChart', 'Target Grade Summary Chart', {
      color: (d, i) ->
        colors[d.status_key]
      x: (d) -> d.key
      y: (d) -> d.y
